...
check_health() {
    ...
    if ! docker-compose --env-file .env.production -f "$COMPOSE_FILE" ps | grep -q "Up"; then
        error "Some services failed to start properly"
    fi
    
    # Check database connectivity
    if docker-compose --env-file .env.production -f "$COMPOSE_FILE" exec -T postgres pg_isready -U strategist -d collective_strategist; then
        success "Database is healthy"
    else
        error "Database health check failed"
    fi
    
    # Check Redis connectivity - load password from environment
    source .env.production
    if docker-compose --env-file .env.production -f "$COMPOSE_FILE" exec -T redis redis-cli --no-auth-warning -a "$REDIS_PASSWORD" ping | grep -q "PONG"; then
...
backup_database() {
    ...
    docker-compose --env-file .env.production -f "$COMPOSE_FILE" exec -T postgres pg_dump \
        -U strategist \
...
status() {
    ...
    docker-compose --env-file .env.production -f "$COMPOSE_FILE" top
}
...
logs() {
    local service=${1:-}
    if [[ -n "$service" ]]; then
        docker-compose --env-file .env.production -f "$COMPOSE_FILE" logs -f "$service"
    else
